...
function doLogin(login,pw){
 ...
 if(!u)return{error:'Нет такого логина (сервер мог перезапускаться — зарегистрируйтесь заново).'};
...
function newRoom(code,login,name){
 return{code,names:[name,null],rtok:[tok(12),null],
  ...
  pendingSkip:null,skipEv:null,winner:null,note:null,deck:[],used:{},v:0,
  stats:[{solved:0,sum:0,best:null,wrong:0},{solved:0,sum:0,best:null,wrong:0}],
  q:[null,null],seen:[now(),0]};
}
function view(r,seat){
 return{code:r.code,seat,names:r.names,online:[!!r.q[0],!!r.q[1]],
  ...
  bankN:bank.length,servertime:now()};
}
function broadcast(r){
 for(let s=0;s<2;s++){
  const q=r.q[s];if(!q)continue;
  try{q({t:'snap',snap:view(r,s)})}catch(e){r.q[s]=null}
 }
}
function finishRound(r,kind,ms){
 ...
 r.round=null;r.pendingSkip=null;r.v++;broadcast(r);
}
function resetMatch(r){
 Object.assign(r,{scores:[0,0],hist:[],tries:[0,0],n:0,round:null,deck:[],used:{},winner:null,
  ...
  stats:[{solved:0,sum:0,best:null,wrong:0},{solved:0,sum:0,best:null,wrong:0}]});
 r.phase='between';r.between={until:now()+1500,last:null,finalAfter:false};r.v++;broadcast(r);
}
function pickTask(r){
...
function newRound(r){
 ...
 if(!task){r.phase='lobby';r.between=null;
  r.note='Нет задач: пополните банк или включите темы генераторов.';r.v++;broadcast(r);return}
 r.n++;
 ...
 for(const r of rooms.values()){
  ...
  if(r.phase==='between'&&r.between&&t>=r.between.until){
   if(r.between.finalAfter){r.phase='final';r.winner=r.scores[0]>=r.settings.target?0:1;
    r.between=null;r.v++;console.log('■ комната',r.code,'— матч',r.scores.join(':'));broadcast(r)}
   else newRound(r);
   continue;
  }
  if(r.pendingSkip&&t>=r.pendingSkip.until){
   r.skipEv={res:'timeout',to:r.pendingSkip.by,n:r.pendingSkip.n};
   r.pendingSkip=null;r.v++;broadcast(r);
  }
 }
},1000);
...
setInterval(()=>{
 ...
 for(const[code,r]of rooms){
  ...
  if(!r.q[0]&&!r.q[1]&&t-last>2*3600*1000){rooms.delete(code);console.log('− комната',code,'удалена (простой)')}
 }
},10*60*1000).unref();
...
function act(d){
 ...
 switch(a){
  case 'settings':{
   ...
   r.v++;broadcast(r);return{ok:true};
  }
  case 'start':{
  ...
  case 'answer':{
   ...
   r.tries[seat]++;
   if(!good){r.stats[seat].wrong++;r.v++;broadcast(r);return{ok:false,msg:'неверно'}}
  ...
  case 'skip':{
   ...
   r.pendingSkip={by:seat,n:rd.n,until:now()+15000};r.v++;broadcast(r);return{ok:true};
  }
  case 'skipReply':{
   ...
   else{r.skipEv={res:'no',to:p.by,n:p.n};r.pendingSkip=null;r.v++;broadcast(r)}
   return{ok:true};
  }
  case 'rematch':{
...
const server=http.createServer(async(req,res)=>{
 ...
 if(req.method==='GET'){
  ...
  if(u.pathname==='/events')return sse(req,res,u.searchParams.get('rtok')||'');
...
function sse(req,res,rtok){
 const{r,seat}=findRoom(rtok);
 if(!r){res.writeHead(403);return res.end()}
 res.writeHead(200,{'Content-Type':'text/event-stream; charset=utf-8','Cache-Control':'no-cache',
  'X-Accel-Buffering':'no'});
 const q=m=>{try{res.write('data: '+JSON.stringify(m)+'\n\n')}catch(e){}};
 r.q[seat]=q;r.seen[seat]=now();
 q({t:'snap',snap:view(r,seat)});
 broadcast(r); // чтобы у соседа зажглась точка «онлайн»
 const hb=setInterval(()=>{try{res.write(': hb\n\n')}catch(e){}},15000);
 req.on('close',()=>{clearInterval(hb);if(r.q[seat]===q){r.q[seat]=null;broadcast(r)}});
}
server.listen(PORT,'0.0.0.0',()=>{
 console.log('ТЕОРЕМА: слушаю порт '+PORT);
});